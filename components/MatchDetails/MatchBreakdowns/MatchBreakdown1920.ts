...
export default class MatchBreakdown1920 {
  getRows(match: Match): MatchBreakdownRow[] {
    const details: SkystoneMatchDetails = new SkystoneMatchDetails().fromJSON(
    ...
    return [
      ...
      MatchBreakdownBoolField(
        'Repositioning Foundation',
        red.foundationRepositioned,
        blue.foundationRepositioned,
        10
      ),
      MatchBreakdownField(
        'Delivering Skystones',
        red.autoDeliveredSkystones,
        blue.autoDeliveredSkystones,
        10
      ),
      MatchBreakdownField(
        'Delivering Stones under Alliance Skybridge',
        red.autoDeliveredStones,
        blue.autoDeliveredStones,
        2
      ),
      MatchBreakdownField('Stones on Foundation', red.autoPlaced, blue.autoPlaced, 4),
      MatchBreakdownField(
        'Navigating under Skybridge',
        this.getNavigatingRobots(red),
        this.getNavigatingRobots(blue),
        5
      ),
      ...
      ...(red.autoReturned > 0 || blue.autoReturned > 0
        ? [
            ...
            MatchBreakdownBoolField(
              'First returned Skystone',
              red.firstReturnedIsSkystone,
              blue.firstReturnedIsSkystone,
              -8
            )
          ]
        : []),
