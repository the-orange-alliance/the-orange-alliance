...
const getHighScoreMatch = (type: 'all' | 'elims' | 'quals' | 'single_team'): Promise<Match> => {
  return new Promise<Match>(async (resolve, reject) => {
    try {
      const match: Match = await TOAProvider.getAPI().getHighScoreMatch(type, { seasonKey: CURRENT_SEASON });
...
const parseHomeProps = (props: IRawHomeProps): IHomeProps => {
  ...
  overall.participants = props.overallHighScoreParticipants.map((p: any) => new MatchParticipant().fromJSON(p));

  const quals = new Match().fromJSON(props.qualsHighScoreMatch);
  quals.event = new Event().fromJSON(props.qualsHighScoreEvent);
  quals.participants = props.qualsHighScoreParticipants.map((p: any) => new MatchParticipant().fromJSON(p));

  const elims = new Match().fromJSON(props.elimsHighScoreMatch);
  elims.event = new Event().fromJSON(props.elimsHighScoreEvent);
  elims.participants = props.elimsHighScoreParticipants.map((p: any) => new MatchParticipant().fromJSON(p));
...
const getHomeData = async (): Promise<IRawHomeProps> => {
  ...
  return {
    ...
    overallHighScoreParticipants: homePageResults[2].participants.map(p => undefinedToNull(p.toJSON())),
    qualsHighScoreParticipants: homePageResults[3].participants.map(p => undefinedToNull(p.toJSON())),
    elimsHighScoreParticipants: homePageResults[4].participants.map(p =>undefinedToNull( p.toJSON())),
  }
}
