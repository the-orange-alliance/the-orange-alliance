            ...
            <CardContent className="w-100 p-3">
              ...
              <Box>
                <Select
                  ...
                  variant={"standard"}
                  onChange={(val: SelectChangeEvent<number>) => pushNewFilter(seasons[val.target.value as number])}
                >
                  {
                    seasons.map((season, i) => (
                      <MenuItem value={i} key={season.seasonKey}>{season.description}</MenuItem>
                    ))
                  }
                </Select>
                ...
                <ul className="nav flex-column mt-2" style={{padding: "0 12px!important"}}>
                  <li>
                    <a className={'link'} rel={"noreferrer"} style={{cursor: 'pointer'}}>{t('pages.team.team_info')}</a>
                  </li>
                    ...
                    <ul className="nav flex-column p-0">
                      <li className="nav-item text-primary">
                        {team.events.map(event => (
                          <a key={event.eventKey} rel={"noreferrer"} className="nav-link" style={{cursor: 'pointer'}}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="d-flex align-items-center mb-1">
                ...
                <div>
                  <Typography variant={'h5'}>#{team.teamNumber} - {team.teamNameShort}</Typography>
              ...
              <Box className="pt-3">
                <Box className={'mb-2'}>
                  ...
                  <Typography display={'inline'}
                              variant={'body1'}>{t('pages.team.part_of_region').replace('{{ regionKey }}', team.regionKey)} </Typography>
                </Box>

                <Box className={'mb-2'}>
                  <Room className={'me-1'} color={"primary"}/>
                  <Typography display={'inline'} style={{textTransform: 'capitalize'}}><a rel={'noreferrer'} href={`
                    https://www.google.com/maps/search/?api=1&query=${(team.city + ', ' + (team.stateProv ? team.stateProv + ', ' : '') + team.country).replace(' ', '+')}`}
                                                                                          target="_blank">{team.city + ', ' + (team.stateProv ? team.stateProv + ', ' : '') + team.country}</a></Typography>
                </Box>

                {team.website && getUrl().startsWith('facebook.com/') &&
                <Box className={'mb-2'}>
                  <Facebook className={'me-1'} color={"primary"}/>
                  <Typography display={'inline'}>Facebook: <a
                    href={team.website}>{getUrl().replace("www.facebook.com/", "")}</a></Typography>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography display={'inline'}>{t('pages.team.website')}: <a
                    href={team.website}>{getUrl()}</a></Typography>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography display={'inline'}>{t('pages.team.rookie_year')}: {team.rookieYear}</Typography>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography
                    display={'inline'}><b>{wlt.wins}-{wlt.losses}-{wlt.ties}</b> (W-L-T) {getSeasonString(selectedSeason)} season</Typography>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography display={'inline'}><b>{topOpr.opr}</b> Top OPR</Typography> <p
                  className="nav-item text-primary clickable"> (Jump)</p>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography
                    display={'inline'}>{t('pages.team.last_active')}: {getSeasonString(lastActive)}</Typography>
                </Box>
                }
                ...
                <Box className={'mb-2'}>
                  ...
                  <Typography
                    display={'inline'}>{t('pages.team.awards_in_season').replace('{{ awards }}', team.awards.length).replace('{{ season }}', selectedSeason.description)}</Typography>
                </Box>
                }
              </Box>
            </CardContent>
                  ...
                  <CardContent>
                    <Box>
                      <Link href={`/events/${event.eventKey}/rankings`}>
                        <Typography
                          variant={'h5'}><b>{event.divisionName ? event.eventName + ' - ' + event.divisionName + ' Division' : event.eventName}</b></Typography>
                      </Link>
                      <Typography
                        variant={"subtitle2"}>{event.city}, {event.stateProv ? event.stateProv + ', ' : ''}{event.country} on {readableDate(event.startDate)}</Typography>
                      <Divider className={'mb-4'}/>
                      {event.rankings[0] &&
                      <Typography variant={'body2'} className={'mb-1 mt-1'}>
                        <b>Qual Rank #{event.rankings[0].rank}</b> with a record
                        of <b>{event.rankings[0].wins}-{event.rankings[0].losses}-{event.rankings[0].ties}</b>
                        {event.rankings[0] &&
                        <a> and an OPR of <b>{event.rankings[0].opr}</b></a>
                        }
                      </Typography>
                      }
...
export const getServerSideProps: GetServerSideProps = async (context: GetServerSidePropsContext) => {
  try {
    const seasonKey = (context.query.season_key && typeof context.query.season_key === 'string') ? context.query.season_key : CURRENT_SEASON;
